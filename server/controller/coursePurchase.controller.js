...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        const newPurchase = new CoursePurchase({
            ...
            amount: course.coursePrice,
            status: "pending",
            orderId: order.id,  // <-- Correctly saving orderId now
            paymentId: null,    // <-- Will be filled after successful payment
        });
        ...
        const successUrl = `${frontendUrl}/course-progress/${courseId}`;
        const cancelUrl = `${frontendUrl}/course-detail/${courseId}`;
...
export const razorpayWebhook = async (req, res) => {
    ...
    try {
        const expectedSignature = crypto
            .createHmac("sha256", secret)
            .update(rawBody.toString('utf8')) // Corrected!
            .digest("hex");
        ...
        const purchase = await CoursePurchase.findOne({ orderId: order_id }).populate("courseId");
        if (!purchase) return res.status(404).json({ message: "Purchase not found" });
        ...
        purchase.status = "completed";
        purchase.amount = amount / 100; // converting back from paise
        purchase.paymentId = paymentId;
        await purchase.save();

        // Optional: Unlock lecture previews
        if (purchase.courseId?.lectures?.length) {
            await Lecture.updateMany(
                { _id: { $in: purchase.courseId.lectures } },
                { $set: { isPreviewFree: true } }
