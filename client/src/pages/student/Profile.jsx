...
const Profile = () => {
  const { data, isLoading, isError } = useLoadUserQuery();  // Fetch user data once
  const [name, setName] = useState("");
  const [profilePhoto, setProfilePhoto] = useState("");
  const [updateUser, { isLoading: updateUserIsLoading, isSuccess, isError: updateError }] = useUpdateUserMutation();
  ...
  // Update user profile and refresh page if successful
  useEffect(() => {
    if (isSuccess) {
      toast.success('Profile updated.');
      window.location.reload();  // Refresh the page to reflect updates immediately
    }
    if (updateError) {
